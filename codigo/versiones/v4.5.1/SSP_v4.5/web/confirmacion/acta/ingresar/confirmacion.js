...
function validar(F){
	...
	if(!vacio(F.num_sacramento.value)){
		alert("Debe ingresar el NÚMERO DEL ACTA de Confirmación.")
		return false
	}
	if(!vacio(F.anio_libro.value)){
		alert("Debe ingresar el A\ÑO DEL LIBRO del Acta de Confirmación.")
		return false
	}
	...
	if(!vacio(F.num_libro.value)){
		alert("Debe ingresar el NÚMERO DEL LIBRO del Acta de Confirmación.")
		return false
	}
	if(!vacio(F.folio_libro.value)){
		alert("Debe ingresar el NÚMERO DEL FOLIO del Acta de Confirmación.")
		return false
	}
	...
	if(!comprobarFechas(F.fecha_nacimiento.value, F.fecha_confirmacion.value)){
		alert("No puede ser la FECHA DE NACIMIENTO mayor a la FECHA DE CONFIRMACIÓN.")
		return false
	}	
	else{
		ok()
