...
BEGIN
  ...
  ELSE
    ...
    ELSE
      IF(tipodoc='confirmacion')THEN
        UPDATE Confirmar SET estado = 0 WHERE id_confirmado = id_;
      END IF;
    END IF;
  END IF;
...
BEGIN
  ...
  SELECT re.anio_libro, re.num_libro, re.folio_libro, re.num_sacramento,
         ...
         re.id_registro_eclesiastico, m.id_ministro, pc.observacion, pc.estado
...
BEGIN
  ...
  IF(tipodoc='bautizo')THEN
    SELECT solicitante.id_solicitante, solicitante.nombre, solicitante.apellido
    FROM bautizar, registroeclesiastico, solicitante
    WHERE bautizar.id_registro_eclesiastico=registroeclesiastico.id_registro_eclesiastico AND bautizar.id_bautizado=solicitante.id_solicitante AND registroeclesiastico.fecha_bautizo= fecha_ AND bautizar.estado = 1;
  ELSE
    IF(tipodoc='comunion')THEN
      SELECT solicitante.id_solicitante, solicitante.nombre, solicitante.apellido
      FROM solicitante, comulgarporprimeravez
      WHERE solicitante.id_solicitante=comulgarporprimeravez.id_comulgado AND comulgarporprimeravez.fecha_comunion=fecha_ AND comulgarporprimeravez.estado = 1;
    ELSE
      IF(tipodoc='confirmar')THEN
        SELECT solicitante.id_solicitante, solicitante.nombre, solicitante.apellido
        FROM solicitante, confirmar
        WHERE solicitante.id_solicitante=confirmar.id_confirmado AND confirmar.fecha_confirmacion=fecha_ AND confirmar.estado = 1;
      ELSE
        IF(tipodoc='matrimonio')THEN
          SELECT m.id_matrimonio, m.id_esposo, m.id_esposa, eo.nombre, eo.apellido, ea.nombre, ea.apellido
          FROM casar m, solicitante eo, solicitante ea
          WHERE m.id_esposo = eo.id_solicitante AND m.id_esposa = ea.id_solicitante  AND m.fecha_matrimonio= fecha_ AND m.estado = 1;
        END IF;
      END IF;
    END IF;
  END IF;
...
/*!50003 SET @TEMP_SQL_MODE=@@SQL_MODE, SQL_MODE='' */ $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `ssp_existe_ministro`(nombre_ VARCHAR(50), apellido_ VARCHAR(50), dignidad_ VARCHAR(50), OUT mensaje VARCHAR(10))
BEGIN
  DECLARE apellido_ministro VARCHAR(50);
  SELECT apellido INTO apellido_ministro FROM Ministro WHERE nombre = nombre_ AND apellido = apellido_ AND dignidad = dignidad_;
...
BEGIN
  ...
  SET AUTOCOMMIT= 0;
  CALL ssp_existe_ministro(nombre_, apellido_, dignidad_, salida);
...
BEGIN
  ...
  UPDATE casar c
  SET c.fecha_matrimonio=fechaMatrimonio_, c.lugar_matrimonio=lugarMatrimonio_, c.observacion=observacion_, c.estado=1, id_ministro=id_ministro_
	WHERE id_matrimonio=id_matrimonio_;
